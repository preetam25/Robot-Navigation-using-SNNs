function [membranePotential, spikeFlags] = LIF (Iapplied_prev, Iapplied, membranePotential, timestep, dt, VT, EL, VTL)
    ...
    % Perform simulation
     ...
     for j = 1:N
          ...
          if(timestep>=2)
              if((membranePotential(j,1)==VT)||(membranePotential(j,2)==VT)) 
                  membranePotential(j,3)=EL; 
              end
          end
          ...
          if(membranePotential(j,3) <= VTL) 
              membranePotential(j,3)=VTL;
          end
