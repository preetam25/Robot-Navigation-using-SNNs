...
LMN = zeros(1000,10000);
DTN = zeros(1000,10000);
...
for i=1:1:1000
    for j=1:1:1000
        ...
        if(weights_LMN_to_DTN(i,j)<(0.75*We_max))
            weights_LMN_to_DTN(i,j) = 0;
        end
        if(weights_DTN_to_LMN(i,j)<(0.75*Wi_max))
...
for i=1:1:1000
    for j=1:1:500
        ...
        if(lateral_connection_weights(i,j)<(0.75*Wl_max))
...
%define omega here
omega = 30*1e-9*ones(1,10000);


background_current_LMN = ones(1000,10000);
background_current_DTN = ones(1000,10000);
...
synaptic_current_LMN = zeros(1000,10000);
synaptic_current_DTN = zeros(1000,10000);

spike_times_LMN = zeros(1000,10000);
spike_times_DTN = zeros(1000,10000);
S = zeros(1000,10000);
for j = 1:10
    for i = 1:1000
...
V_LMN = zeros(1000,10000);
V_DTN = zeros(1000,10000);
I_LMN = zeros(1000,10000);
I_DTN = zeros(1000,10000);
